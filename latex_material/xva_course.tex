...
\begin{frame}
    ...
    CVA introduces a new source of risk for banks, as changes in counterparty credit spreads or market parameters can lead to significant CVA P\&L (Profit \& Loss) volatility. Banks actively manage and hedge their CVA exposures.
    ...
    \end{itemize}
        ...
        *   **Accounting Mismatches:** Differences in accounting treatment between derivatives and hedging instruments can create P\&L volatility.
...
\begin{frame}
    ...
    FVA = Sum[i=1 to T] (Funding\_Cost\_Benefit\_i * DF\_i)

    Where `Funding\_Cost\_Benefit\_i` depends on the expected exposure and the bank\'s funding spread.
...
\begin{frame}
    \subsection*{Topic 4.4: The FVA Debate: Is it a Real Cost?}
    \frametitle{Topic 4.4: The FVA Debate: Is it a Real Cost?}

    The inclusion of FVA in derivative pricing has been debated. Critics argue that funding costs are already reflected in a bank's overall cost of capital, while proponents note that uncollateralized trades consume funding resources that should be explicitly priced.

    \begin{itemize}
    \item **Proponents:** FVA captures the opportunity cost of funding positive exposures and the benefit of funding negative exposures.
    \item **Critics:** Some view FVA as double-counting when credit spreads already embed funding effects.
    \item **Industry Practice:** Most major banks now include FVA, but methodologies vary.
    \end{itemize}
\end{frame}

\begin{frame}
    \subsection*{Topic 4.5: FVA Hedging and Management}
    \frametitle{Topic 4.5: FVA Hedging and Management}

    Managing FVA involves balancing funding strategies and liquidity risk.

    \begin{itemize}
    \item **Internal Funding Desks:** Coordinate funding for trading books and optimize collateral usage.
    \item **Hedging Tools:** Banks may use term funding, repos, or structured notes to manage funding exposure.
    \item **Challenges:** Funding spreads can be volatile, and hedging instruments may introduce additional basis and liquidity risks.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    MVA = Sum[i=1 to T] (EIM\_i * Funding\_Spread * dt * DF\_i)
...
\begin{frame}
    \subsection*{Topic 5.4: MVA Hedging and Management}
    \frametitle{Topic 5.4: MVA Hedging and Management}

    Hedging MVA is challenging because initial margin requirements depend on portfolio composition and market volatility.

    \begin{itemize}
    \item **Margin Optimization:** Banks may restructure trades or use margin-efficient products to reduce initial margin needs.
    \item **Funding Strategies:** Dedicated funding for initial margin, such as collateralized borrowing, can reduce costs but introduces liquidity considerations.
    \item **Operational Challenges:** Frequent recalculation of IM and settlement cycles add complexity.
    \end{itemize}
\end{frame}

\begin{frame}
    \subsection*{Topic 5.5: Regulatory Impact: Driving Behavioral Change}
    \frametitle{Topic 5.5: Regulatory Impact: Driving Behavioral Change}

    Regulatory reforms mandating initial margin have reshaped derivatives markets.

    \begin{itemize}
    \item **BCBS-IOSCO Framework:** Phased implementation has brought more counterparties into scope, increasing demand for high-quality collateral.
    \item **Product Innovation:** The cost of posting IM has encouraged the use of cleared products and compression services.
    \item **Strategic Considerations:** Institutions evaluate trade profitability after accounting for MVA, influencing deal structures and client pricing.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    As financial markets evolve, XVA practices continue to develop.

    \begin{itemize}
    \item **Model Risk and Automation:** Increased use of machine learning and automated risk engines raises governance and validation challenges.
    \item **Central Clearing Expansion:** Wider adoption of central clearing and standardized products may reduce certain XVA components while increasing others like MVA.
    \item **Sustainability Considerations:** Emerging regulatory focus on ESG factors could influence funding costs and capital requirements, impacting FVA and KVA.
    \end{itemize}
\end{frame}
